...
client:on('messageCreate', function(message)
	...
	if not ((not config.limitToGuild) or (config.limitToGuild == "")) then
		if istable(config.limitToGuild) then
			local validGuild = false

			for k, v in ipairs(config.limitToGuild) do
				if message.guild.id == v then -- Check if the guild matches the whitelist
					validGuild = true
					break
				end
			end

			-- Not a whitelisted guild
			if not validGuild then return end
		else
			-- Check against the approved guild
